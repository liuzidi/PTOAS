...
- VfSim 当前能够识别 `vlds`、`vsts`，但对带 `POST_UPDATE` 语义的形式预测还不够
  准确。隐式地址更新、地址依赖及其相关开销需要在 VfSim adapter 和模型中继续完善。
- VfSim 当前不建模 predicate register。mask/predicate 信息只用于恢复有效 lane 和指令
  语义，不预测 predicate register 的分配、生命周期、资源冲突及相关时序开销。
- 上述能力的后续完善主要位于 VfSimulator 的 low-level VPTO adapter 和核心模型侧。
  在本文 IR/API 协议保持不变的前提下，PTOAS 只需更新 VfSimulator submodule commit，
  并补充相应的 lit、端到端及 camodel 对齐测试，无需修改 planner 接入位置和 pass 链路。
