...
static bool hasUnmodeledMemoryAccess(scf::ForOp loop) {
  ...
  loop.getBody()->walk([&](Operation *op) {
    ...
    if (auto store = dyn_cast<pto::VMIvStoreOp>(op)) {
      // post_update stores carry a loop-carried updated_base SSA chain;
      // their moving destination cannot be resolved to a compile-time
      // UB, but that is expected and does not block fusion.
