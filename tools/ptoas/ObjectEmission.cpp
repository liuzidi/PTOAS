...
static bool compileDeviceLLVMToObject(llvm::StringRef llPath,
  ...
  if (options.disableBishengVFFusion) {
    // PTOAS VMI fusion has already handled these decisions. Keep Bisheng's
    // independent VF pipeline from optimizing memory traffic a second time.
...
mlir::LogicalResult mlir::pto::emitFatobjLLVM(
  ...
  if (!artifacts.emitVectorObject(vectorModule, toolchain,
                                  vfsimtSizeFixMode, diagOS, options))
    return failure();
