...
static LogicalResult collectVPTOKernelStubDecls(
  ...
  for (ModuleOp module : modules) {
    module.walk([&decls, &logicalNameToIndex, &hadError, &diagOS,
      ...
      // ``pto.entry`` by vpto-split-cv-module before this runs; helpers that
      // merely carry ``pto.kernel_kind`` (verifiers read it off the enclosing
      // func) must stay non-entries.
...
LogicalResult mlir::pto::emitVPTODeviceWrapperSource(
  ...
  if (stubDecls.empty()) {
    diagOS << "Error: no PTO entry functions found for device wrapper "
              "emission. Entries must carry an explicit `pto.entry` (or "
              "`pto.kernel`) attribute; a module whose functions only carry "
              "`pto.kernel_kind` gets `pto.entry` stamped automatically only "
              "when exactly one unreferenced public kind-tagged definition "
              "exists (the PyPTO single-kernel-per-file shape). Modules with "
              "multiple kind-tagged entry candidates or only private/called "
              "helpers must mark entries explicitly.\n";
    return failure();
  }
  if (stubDecls.size() != 1) {
    diagOS << "Error: --vpto-emit-merged-device-only requires exactly "
               "one PTO entry function per device ELF (the PyPTO "
               "single-kernel-per-file shape); got "
            << stubDecls.size()
            << ". Split the input per kernel kind (one entry per file) or "
               "use the regular fatobj route, which supports multiple "
               "entries via <<<>>> launch stubs.\n";
    return failure();
  }
